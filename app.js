(() => {
  ...
  function row(name,value){
    ...
    state.className=`state ${k}`;
    state.querySelector("span").textContent=label(k);
  }
  ...
  async function refresh(){
    try{
      ...
      row("discord",{status:bot?"online":"offline"});
      const s=d.services||{};
      row("youtube",s.youtube||"unknown");
      row("tiktok",s.tiktok||"unknown");
      row("twitch",s.twitch||"unknown");
